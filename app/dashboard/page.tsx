...
export default function DashboardPage() {
  ...
  return (
    <main style={{
      ...
      {/* MAIN PANEL */}
      <div style={{...T.panel, width:"100%",maxWidth:1020,borderRadius:36,position:"relative",overflow:"hidden",zIndex:1,transform:"perspective(1400px) rotateX(0.8deg) translateY(-2px)"}}>
        <div style={shine}/>

        {/* NAV */}
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"13px 28px 12px",borderBottom:T.border}}>
          ...
          <div style={{display:"flex",gap:20,alignItems:"center"}}>
            {([["Menú","#menu"],["Progreso","/progreso"],["Bienestar","/bienestar"],["Lista compra","/lista-compra"]] as [string,string][]).map(([l,h],i)=>(
              <Link key={l} href={h} style={{fontSize:13,color:i===0?"#3C6020":T.text.label,fontWeight:i===0?500:400,textDecoration:"none",letterSpacing:"0.01em"}}>{l}</Link>
            ))}
            <button onClick={toggle}
              style={{...T.btnW, padding:"8px 12px", fontSize:14, borderRadius:12, marginRight:4}}>
              {theme === "dark" ? "☀️" : "🌙"}
            </button>
            <button onClick={async()=>{await supabase.auth.signOut();router.push("/login");}}
              style={{...T.btnM,padding:"8px 18px",fontSize:12,letterSpacing:"0.02em"}}>
              Salir
            </button>
        ...
        <div style={{display:"grid",gridTemplateColumns:"148px 1fr 196px"}}>
          ...
          <div style={{
            ...
            <div style={{textAlign:"center",paddingBottom:18,borderBottom:T.border,marginBottom:14}}>
              ...
              <p style={{fontSize:10,fontWeight:400,color:T.text.muted,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:3}}>{getSaludo()}</p>
              <p style={{fontFamily:"var(--font-playfair,serif)",fontSize:17,fontWeight:600,color:T.text.primary,letterSpacing:"-0.4px",fontStyle:"italic"}}>{nombre}</p>
              <div style={{display:"inline-flex",alignItems:"center",gap:4,marginTop:6,...T.pill,padding:"3px 10px"}}>
            ...
            {([["◉","Dashboard","/dashboard",true],["◈","Menú","#menu",false],["◎","Perfil clínico","/onboarding",false],["↗","Progreso","/progreso",false],["✦","Bienestar","/bienestar",false],["💬","Nuria","/chat",false]] as [string,string,string,boolean][]).map(([ic,lb,hr,a])=>(
              <Link key={lb} href={hr} style={{display:"flex",alignItems:"center",gap:8,padding:"9px 10px",borderRadius:12,cursor:"pointer",background:a?"rgba(94,136,66,0.12)":"transparent",border:`1px solid ${a?"rgba(94,136,66,0.22)":"transparent"}`,marginBottom:2,textDecoration:"none"}}>
                <span style={{fontSize:12,color:a?"#3C6020":T.text.label,flexShrink:0}}>{ic}</span>
                <span style={{fontSize:12,fontWeight:a?500:400,color:a?"#3C6020":T.text.muted,letterSpacing:"0.01em",whiteSpace:"nowrap"}}>{lb}</span>
              </Link>
            ))}

            <div style={{marginTop:"auto",paddingTop:12}}>
              <div style={{...T.dark,borderRadius:16,padding:14,position:"relative",overflow:"hidden"}}>
          ...
          {/* CENTER */}
          <div style={{padding:"22px 22px",display:"flex",flexDirection:"column",gap:14,borderRight:T.border}}>

            {/* Hero header */}
            <div style={{display:"flex",alignItems:"flex-end",justifyContent:"space-between"}}>
              <div>
                <p style={{fontSize:10,fontWeight:500,letterSpacing:"0.14em",textTransform:"uppercase",color:T.text.label,marginBottom:6}}>
                  {new Date().toLocaleDateString("es-ES",{weekday:"long",day:"numeric",month:"long"})}
                </p>
                <h1 style={{fontFamily:"var(--font-playfair,serif)",fontSize:32,color:T.text.primary,letterSpacing:"-1px",lineHeight:1,fontWeight:600}}>
                  Tu plan de hoy
                </h1>
              </div>
              <div style={{display:"flex",gap:7,paddingBottom:4}}>
                <div style={{...T.pill,display:"flex",alignItems:"center",gap:5,padding:"5px 13px"}}>
                  ...
                  <span style={{fontSize:11,fontWeight:500,color:T.text.primary}}>{checkinDone?"Check-in ✓":"Sin check-in"}</span>
                </div>
                <Link href="/lista-compra" style={{...T.pill,display:"flex",alignItems:"center",gap:5,padding:"5px 13px",textDecoration:"none"}}>
                  <span style={{fontSize:11,fontWeight:500,color:T.text.primary}}>Lista compra ◎</span>
                </Link>
              </div>
            </div>
            ...
            {diaMenu && (
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8}}>
                {[
                  ...
                  <div key={label} style={{...T.card,borderRadius:16,padding:"12px 14px",position:"relative",overflow:"hidden"}}>
                    <div style={shine}/>
                    <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:`linear-gradient(90deg,${color},transparent)`,opacity:0.6}}/>
                    <p style={{fontSize:9,fontWeight:600,color:T.text.label,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:8}}>{label}</p>
                    <p style={{fontFamily:"var(--font-playfair,serif)",fontSize:22,fontWeight:600,color:T.text.primary,letterSpacing:"-1px",lineHeight:1}}>{value}</p>
                    <p style={{fontSize:10,color:T.text.label,marginTop:4,fontWeight:300}}>{sub}</p>
                  </div>
                ))}
              </div>
            )}
            ...
            {/* Check-in */}
            {!checkinDone && (
              <div style={{...T.card,borderRadius:20,padding:"14px 18px",position:"relative",overflow:"hidden"}}>
                <div style={shine}/>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <div>
                    <p style={{fontSize:13,fontWeight:600,color:T.text.primary,letterSpacing:"-0.3px"}}>Check-in de hoy</p>
                    <p style={{fontSize:11,color:T.text.muted,marginTop:3,fontWeight:300}}>Cuéntale a Nuria cómo estás</p>
                  </div>
                  <button onClick={()=>setShowCheckin(!showCheckin)}
                    style={{...T.btnM,padding:"8px 18px",fontSize:12}}>
            ...
            {/* Menú */}
            <div id="menu" style={{...T.card,borderRadius:20,padding:"18px 20px",position:"relative",overflow:"hidden",flex:1}}>
              <div style={shine}/>
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:16}}>
                <div>
                  <p style={{fontSize:14,fontWeight:600,color:T.text.primary,letterSpacing:"-0.3px"}}>Menú semanal</p>
                  {menu && <p style={{fontSize:10,color:T.text.label,marginTop:2,fontWeight:300}}>Generado por Nuria</p>}
                </div>
                {menu && (
                  <button onClick={handleGeneratePlan} disabled={generating} style={{...T.btnW,padding:"7px 16px",fontSize:11}}>
              ...
              {!menu ? (
                <div style={{textAlign:"center",padding:"36px 20px",background:"rgba(94,136,66,0.04)",borderRadius:16,border:"1.5px dashed rgba(94,136,66,0.18)"}}>
                  ...
                  <p style={{fontFamily:"var(--font-playfair,serif)",fontSize:17,color:T.text.primary,fontStyle:"italic",marginBottom:8}}>Aún no tienes menú</p>
                  <p style={{fontSize:12,color:T.text.muted,fontWeight:300,marginBottom:20}}>Nuria generará tu plan en segundos</p>
                  <button onClick={handleGeneratePlan} disabled={generating}
                    style={{...T.btnM,padding:"12px 28px",fontSize:13,borderRadius:16}}>
                  ...
                  <div style={{display:"flex",gap:5,marginBottom:16}}>
                    {DIAS_SHORT.map((d,i)=>(
                      <button key={d} onClick={()=>setDiaIdx(i)}
                        ...
                        <p style={{fontSize:11,fontWeight:diaIdx===i?600:400,color:diaIdx===i?"white":T.text.muted}}>{d}</p>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:9}}>
                      {[
                        ...
                        <div key={tipo} style={{
                          ...
                          borderRight:expandido===`${diaIdx}-${tipo}`?"1px solid rgba(94,136,66,0.18)":T.card.borderRight,
                          borderBottom:expandido===`${diaIdx}-${tipo}`?"1px solid rgba(94,136,66,0.12)":T.card.borderBottom,
                          position:"relative",
                        }}>
                          ...
                          <button onClick={()=>setExpandido(expandido===`${diaIdx}-${tipo}`?null:`${diaIdx}-${tipo}`)}
                            ...
                            <div style={{display:"flex",alignItems:"center",gap:12}}>
                              ...
                              <div>
                                <p style={{fontSize:9,fontWeight:600,color:T.text.label,textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:4}}>{tipo}</p>
                                <p style={{fontSize:15,fontWeight:600,color:T.text.primary,letterSpacing:"-0.4px"}}>{plato.nombre}</p>
                                <p style={{fontSize:10,color:T.text.label,marginTop:3,fontWeight:300}}>{plato.ingredientes.length} ingredientes</p>
                              </div>
                            </div>
                            <div style={{width:28,height:28,borderRadius:"50%",background:"rgba(255,255,255,0.6)",border:"1px solid rgba(255,255,255,0.9)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                              <span style={{color:T.text.label,fontSize:16,fontWeight:300,lineHeight:1}}>{expandido===`${diaIdx}-${tipo}`?"−":"+"}</span>
                            </div>
                          </button>
                              ...
                              <div style={{paddingTop:12,display:"flex",flexDirection:"column",gap:7}}>
                                {plato.ingredientes.map(ing=>(
                                  <div key={ing.nombre} style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                                    ...
                                    <span style={{fontSize:12,color:T.text.label,fontWeight:300,background:"rgba(94,136,66,0.06)",padding:"2px 8px",borderRadius:8}}>{ing.cantidad_g}g</span>
                                  </div>
                                ))}
                              </div>
                            </div>
          ...
          <div style={{padding:"18px 14px",display:"flex",flexDirection:"column",gap:12}}>
            ...
            {/* NutriScore */}
            <div style={{...T.card,borderRadius:20,padding:18,position:"relative",overflow:"hidden"}}>
              <div style={shine}/>
              <p style={{fontSize:9,fontWeight:600,color:T.text.label,textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:14}}>NutriScore semanal</p>
              <NutriScoreCard />
            </div>

            {/* Accesos */}
            <div style={{...T.card,borderRadius:20,padding:"16px 14px",position:"relative",overflow:"hidden"}}>
              <div style={shine}/>
              <p style={{fontSize:9,fontWeight:600,color:T.text.label,textTransform:"uppercase",letterSpacing:"0.12em",marginBottom:12}}>Accesos rápidos</p>
              {([
                ...
                <Link key={lb} href={hr} style={{display:"flex",alignItems:"center",gap:9,padding:"9px 10px",borderRadius:13,background:bg,border:"1px solid rgba(255,255,255,0.88)",marginBottom:6,cursor:"pointer",textDecoration:"none",boxShadow:"0 2px 8px rgba(30,60,15,0.05),inset 0 1px 0 rgba(255,255,255,0.95)"}}>
                  ...
                  <span style={{fontSize:12,fontWeight:500,color:T.text.primary,letterSpacing:"-0.2px"}}>{lb}</span>
            ...
            {/* Nuria */}
            <div style={{...T.matcha,borderRadius:20,padding:16,position:"relative",overflow:"hidden"}}>
